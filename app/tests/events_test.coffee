...
describe("Event System", ->
  ...
  it("should be able to add nested events", ->
    res = tweak.Events.on(window, "one/two", ->)
    expect(res).to.equal(true)
    res = tweak.Events.on(window, "one/two/three/four", ->)
    expect(res).to.equal(true)
  )

  it("should be able to remove nested events", ->
    res = tweak.Events.off(window, "one/two", ->)
    expect(res).to.equal(true)
    res = tweak.Events.off(window, "one/two/three/four", ->)
    expect(res).to.equal(true)
  )
  ...
  it("should be able to tigger up to maximum allowed calls", ->
    ...
    tweak.Events.trigger("one/two/three/four")
    tweak.Events.trigger("one/two/three/four")
    expect(maxCalls).to.equal(totalCalls)
  )

  it("should be able to tigger only events matching context", ->
    tweak.Events.on(@, "one/two/three", -> expect(false).to.equal(true))
