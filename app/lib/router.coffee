###
  ...
  Adding a route ":section:page" or ":section/:page" attached to the event of "navigation",
  will trigger the "navigation" event and pass the following data {url:"/5/93", data:{section:"5", page:"93"}}
  when the HashState is equivalent to "/#/5/93".

  Adding a optional parameter route ":section?page" or ":section/?page" attached to the event of "navigation",
  will trigger the "navigation" event and pass the following data {url:"/5/6", data:{section:"5", page:"6"}}
  when the HashState is equivalent to "/#/5/6".

  Adding a optional parameter route ":section?page" or ":section/?page" attached to the event of "navigation",
  will trigger the "navigation" event and pass the following data {url:"/5", data:{section:"5"}} when the
  HashState is equivalent to "/#/5".

  Adding a splat route ":section:page/*" or ":section/:page/*" attached to the event of "navigation",
  will trigger the "navigation" event and pass the following data
  {url:"/5/6/www.example.com", data:{section:"5", page:"6", splat:"www.example.com"}} when the HashState is
  equivalent to "/#/5/6/www.example.com".
  ...
  {url:"/blog?id=9836384&light&reply=false", data:{blog:{id:9836384, light:"true", reply:"false"}}}.
###
class tweak.Router extends tweak.EventSystem
  ...
  ###
    ...
    @param [String, Regex] route A string formatted string or Regex.
  ###
  add: (event, route) ->
  ...
  __getQueryData = (segment) ->
    ...
    if query
      # Get the parameterss by spitting on the & character
      params = /([^&]+)&*/.exec query[1]
      if params
